...
def prep_training_data(train_ids: List[int], file_format: str,
                       azimuth_edges, zenith_edges, bin_num):
    # Placeholders
    train_x = None
    train_y = None
    print(train_ids)
    ...
    # Output Encoding
    trn_y_anglecode = y_to_angle_code(
        train_y, azimuth_edges, zenith_edges, bin_num)
...
def create_model(strategy, pulse_count, feature_count, lstm_units,
                 bin_num, learning_rate):
    with strategy.scope():
...
def main():
    ...
    angle_bin_vector_sum_x = (np.sin(angle_bin_azimuth1) -
                             ...
                             ((angle_bin_zenith1 -
                               angle_bin_zenith0) / 2 -
                              (np.sin(2 *2 * angle_bin_zenith0)) / 4)
    ...
    val_x, val_y = prep_validation_data([432, 433, 434], file_format)

    for i in range(30):
        model.load_weights(model_save_path.format(
            feature_count, bin_num, batch_size, i))
        ...
        print()
        print(f'Epoch: {i:.1f} MAE: {mae:.10f}')
